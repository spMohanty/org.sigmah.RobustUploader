...
import org.sigmah.client.SlicedFileService;
	...
	public synchronized FileSliceResponse sendSlice(FileSlice slice) throws IllegalArgumentException {
		...
		try {
			...
			if (slice.getSequenceID() == 1) {
				...
				//TO-DO
				// Add a Data Storage Adapter
				File newFile = new File("DataStore/"+slice.getFileName());
				...
				//TO-DO
				// Add a Data Storage Adapter
				file = new RandomAccessFile("DataStore/"+slice.getFileName(), "rw");
			}

			if (slice.getSliceSize() > 0) {
				// Reset the file length to allow for this chunk received.
				file.setLength((slice.getSequenceID()-1)*slice.getMaxChunkSize() + slice.getSliceSize());
